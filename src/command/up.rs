...
pub fn run(
    ...
    let (installed_features, installed_order) = if newly_created {
        ...
        // Customizations are attached as a container label at creation time
        // (see `create_and_start_container` / the compose override).
        (installed, order)
    } else {
        (
    ...
    let probed_env = if probe != "none" && host::is_known_probe(probe) {
